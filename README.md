...
- **极简 KV 写入**（免费版每天仅 1000 次写入）：KV 中**只持久化功能必需的 3 类键**——`config`（账号/密码/时长，仅增删改时写）、`device:<user>`（一次性设备码，几乎不写）、`session:<user>`（登录态缓存，仅会话变化时写，含 `loginTs` 用于推算在线时长）。**所有展示数据（运行摘要、云电脑状态卡片）一律不写 KV**，改为实时查询。稳态单账号每日写入仅数十次（仅登录/配置变更），远低于限额。
- 运行状态：后台「设置」面板新增一行运行状态，展示自动保活频率、**状态更新于**（最近一次实时查询时间）、**下次运行倒计时**、以及**心跳监测**（距最近一次状态刷新 >2 分钟标黄、>10 分钟标红「可能停摆」），便于一眼判断 Cron 是否还在跑。
...
- 运行摘要/心跳 → 基于最近一次快照时间，前端推算，无需 KV 存储。
