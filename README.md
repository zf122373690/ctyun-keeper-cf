...
- **KV 写入最小化**（免费版每天仅 1000 次写入）：`lastRun` 与 `_meta` 合并到同一键（不再单独写 `lastRunMeta`）；`status:<user>` 与 `session:<user>` 均做「内容未变则跳过写入」；Cron 触发下 `lastRun`/`status` 节流为每 10 分钟才写一次。单账号稳态约 290 次/天，远低于限额。若仍提示超出，多半是**当日额度已被早期版本耗光**，等 UTC 0 点重置或升级付费版（$5/月，10 万次/天）。
- 运行状态：后台「设置」面板新增一行运行状态，展示自动保活频率、上次运行时间与触发方式（定时/手动）、**下次运行倒计时**、以及**心跳监测**（距上次运行 >2 分钟标黄、>10 分钟标红「可能停摆」），便于一眼判断 Cron 是否还在跑。
